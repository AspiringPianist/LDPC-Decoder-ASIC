...
This is "path-unrolled" because a full CN update is not done in one monolithic CN unit; it is accumulated across the path as the message traverses columns. Additionally, because the CN and VN phases are mutually exclusive in time, the underlying datapath in the PE aggressively employs **resource sharing**. Major arithmetic components (like the Min1/Min2 comparators and Offset-Min-Sum subtractors) are time-multiplexed between phases to save silicon area with zero latency overhead.
...
- [pe.sv](pe.sv)
	...
	- Highly area-optimized via datapath resource sharing between CN and VN phases.
