...
module pe #(
  ...
  generate
    for (L = 0; L < N_LAYERS; L = L + 1) begin : gen_layer
      ...
      // Use in_min1/in_min2 (global minimums from previous CN pass)
      wire [MIN_W_p-1:0] chosen_mag = (l_mag == in_min1) ? in_min2 : in_min1;
      wire [MIN_W_p-1:0] chosen_mag_off = (chosen_mag > OFFSET_BETA_p) ?
                                          (chosen_mag - OFFSET_BETA_p) : {MIN_W_p{1'b0}};
