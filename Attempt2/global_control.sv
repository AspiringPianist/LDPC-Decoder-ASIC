...
module global_control #(
  ...
  // OPT: Since NUM_FRAMES is a power of 2, replace expensive signed modulo
  //       (divider + sign correction) with a simple bitmask.
  localparam int FR_BITS = $clog2(NUM_FRAMES_p); // 3 for NUM_FRAMES=8
  genvar c;
  generate
